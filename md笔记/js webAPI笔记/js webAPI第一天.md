...
<body>
    ...
    <script>
        ...
        //box.classList.toggle('one')
         //box.classList.contains('one')      //返回的是布尔值 ，如果有这个类返回true 没有返回false
    </script>
</body>
...
| splice        | 数组方法      | 第一个参数是选中索引号，第二个是删除的个数，第三个是要添加的数组元素 |
| querySelector | 获取一个1元素 | document.querySelector('css选择器')                          |
| classList     | 添加类        | classList.add('active')添加类   .remove('')移除类 toggle('')切换类 有就移除，没有就添加 |
| disabled      | 禁用按钮      | 当disabled=true 禁用按钮 disabled=false 开启按钮             |
| checked       | 复选框勾选    | checked=true  勾选   checked=false   未勾选                  |
|               |               |                                                              |
|               |               |                                                              |
|               |               |                                                              |
